...
public sealed class GoapDefenseProductionSelectionExpectationsEditModeTests
{
    ...
    private static IEnumerable<TestCaseData> DefenseCombinedDriveCases()
    {
        foreach (GoapDefenseLayoutPatternId pattern in GoapDefenseLayoutPatternCatalog.BuildDefenseCombinedDriveSuite())
        {
            int number = GoapDefenseLayoutPatternCatalog.GetNumber(pattern);
            for (int slot = 0; slot <= 2; slot++)
            {
                if (!GoapDefenseProductionSelectionExpectations.DefenseCombinedDrive.TryGetExpectation(
                        pattern,
                        slot,
                        out string expected,
                        out bool shouldEvaluate)
                ...
                yield return new TestCaseData(pattern, slot, expected)
                    .SetName($"DefenseCombinedDrive_#{number:D2}_slot{slot}_{expected}");
            }
        }
    }
    ...
    [TestCaseSource(nameof(DefenseCombinedDriveCases))]
    public void DefenseCombinedDrive_MatchesExpectedAction(
        GoapDefenseLayoutPatternId pattern,
        int slot,
        string expectedAction)
    {
        bool ok = GoapDefenseProductionSelectionExpectations.DefenseCombinedDrive.TryGetExpectation(
            pattern,
            slot,
            out string action,
            out bool shouldEvaluate);
    ...
    [Test]
    public void DefenseCombinedDrive_BaselinePattern_SkipsEvaluation()
    {
        bool ok = GoapDefenseProductionSelectionExpectations.DefenseCombinedDrive.TryGetExpectation(
            GoapDefenseLayoutPatternId.Baseline,
            0,
