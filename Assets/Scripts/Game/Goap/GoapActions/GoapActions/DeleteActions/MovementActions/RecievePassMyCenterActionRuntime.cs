...
public class RecievePassMyCenterActionRuntime : GoapActionRuntime
{
    ...
    public RecievePassMyCenterActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        // // ScriptableObjectを正しい型にキャスト
        // var so = origin as RecievePassMyCenterActionSO;
        // if (so != null)
        // {
        //     // パス受け位置の有効半径を設定
    ...
    public override void Execute(PlayerBlackboard bb)
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassMyCenterActionRuntime)] パス受け移動開始");
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (distance > _goodPositionRadius)
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassMyCenterActionRuntime)] radian: {radian} direction: {direction} distance: {distance}");
        ...
        else
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassMyCenterActionRuntime)] 良い位置に到達: {_targetPosition}");
        }
    }
    ...
    private void UpdateTargetPosition()
    {
        ...
        // 自陣中央を狙う
        // フィールドの中央、自陣側の中腹あたりに基準点を置く
        float targetX = fieldCenter.x;   // フィールド中央
        float targetZ = fieldCenter.z - halfLength * 0.5f;  // 自陣側の中央あたり
    ...
    public override void Cancel()
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassMyCenterActionRuntime)] パス受け移動キャンセル");
    }
} 