...
public class RecievePassEnemyRightSideActionRuntime : GoapActionRuntime
{
    ...
    public RecievePassEnemyRightSideActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        // ScriptableObjectを正しい型にキャスト
        var so = origin as RecievePassEnemyRightSideActionSO;
        if (so != null)
        {
            // パス受け位置の有効半径を設定
    ...
    public override void Execute(PlayerBlackboard bb)
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassEnemyRightSideActionRuntime)] パス受け移動開始");
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (distance > _goodPositionRadius)
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassEnemyRightSideActionRuntime)] radian: {radian} direction: {direction} distance: {distance}");
        ...
        else
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassEnemyRightSideActionRuntime)] 良い位置に到達: {_targetPosition}");
        }
    }
    ...
    private void UpdateTargetPosition()
    {
        ...
        // 敵陣右側（Xプラス、Zプラス）を狙う
        // 中央から見て右1/3分右に寄った位置・敵陣側の1/2分寄った位置に基準点を置く
    ...
    public override void Cancel()
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassEnemyRightSideActionRuntime)] パス受け移動キャンセル");
    }
} 