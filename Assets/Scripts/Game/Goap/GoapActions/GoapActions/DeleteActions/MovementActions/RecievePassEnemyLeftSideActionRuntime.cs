...
public class RecievePassEnemyLeftSideActionRuntime : GoapActionRuntime
{
    ...
    public RecievePassEnemyLeftSideActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        // // ScriptableObjectを正しい型にキャスト
        // var so = origin as RecievePassEnemyLeftSideActionSO;
        // if (so != null)
        // {
        //     // パス受け位置の有効半径を設定
    ...
    public override void Execute(PlayerBlackboard bb)
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassEnemyLeftSideActionRuntime)] パス受け移動開始");
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (distance > _goodPositionRadius)
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassEnemyLeftSideActionRuntime)] radian: {radian} direction: {direction} distance: {distance}");
        ...
        else
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassEnemyLeftSideActionRuntime)] 良い位置に到達: {_targetPosition}");
        }
    }
    ...
    private void UpdateTargetPosition()
    {
        ...
        // 敵陣左側（Xマイナス、Zプラス）を狙う
        // 中央から見て左1/3分左に寄った位置・敵陣側の1/2分寄った位置に基準点を置く
    ...
    public override void Cancel()
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassEnemyLeftSideActionRuntime)] パス受け移動キャンセル");
    }
} 