...
public class RecievePassMyRightSideActionRuntime : GoapActionRuntime
{
    ...
    public RecievePassMyRightSideActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        // ScriptableObjectを正しい型にキャスト
        var so = origin as RecievePassMyRightSideActionSO;
        if (so != null)
        {
            // パス受け位置の有効半径を設定
    ...
    public override void Execute(PlayerBlackboard bb)
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassMyRightSideActionRuntime)] パス受け移動開始");
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (distance > _goodPositionRadius)
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassMyRightSideActionRuntime)] radian: {radian} direction: {direction} distance: {distance}");
        ...
        else
        {
            ...
            DebugLogger.Log($"[{_debugName}(RecievePassMyRightSideActionRuntime)] 良い位置に到達: {_targetPosition}");
        }
    }
    ...
    public override void Cancel()
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassMyRightSideActionRuntime)] パス受け移動キャンセル");
    }
} 