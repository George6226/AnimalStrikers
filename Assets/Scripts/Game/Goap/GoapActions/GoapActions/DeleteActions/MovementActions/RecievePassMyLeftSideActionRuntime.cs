...
public class RecievePassMyLeftSideActionRuntime : GoapActionRuntime
{
    ...
    public RecievePassMyLeftSideActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        // ScriptableObjectを正しい型にキャスト
        var so = origin as RecievePassMyLeftSideActionSO;
        if (so != null)
        {
            // パス受け位置の有効半径を設定
            _goodPositionRadius = so.GoodPositionRadius;
            Debug.Log("LeftGood:" + _goodPositionRadius + " 元:"+so.GoodPositionRadius);
        }
    }
    ...
    public override void Execute(PlayerBlackboard bb)
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassMyLeftSideActionRuntime)] パス受け移動開始");
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        // Debug.Log("distance: " + distance + " goodPositionRadius: " + _goodPositionRadius);
        ...
        if (distance > _goodPositionRadius)
        {
            ...
            // DebugLogger.Log($"[{_debugName}(RecievePassMyLeftSideActionRuntime)] radian: {radian} direction: {direction} distance: {distance}");
            // Debug.Log("radian: " + radian + " direction: " + direction + " distance: " + distance);
            // アクション進行度を更新
        ...
        else
        {
            ...
            // DebugLogger.Log($"[{_debugName}(RecievePassMyLeftSideActionRuntime)] 良い位置に到達: {_targetPosition}");
            // Debug.Log("良い位置に到達");
        }
    }
    ...
    private void UpdateTargetPosition()
    {
        ...
        float targetZ = fieldCenter.z - halfLength * 0.5f;  // 自陣に1/2分寄った位置
        float targetY = _playerBlackboard != null ? _playerBlackboard.PhysicalState.Position.y : 0f;

        // Debug.Log("clamp前 targetX: " + targetX + " targetZ: " + targetZ);

        // フィールド境界内にクランプ
        targetX = Mathf.Clamp(targetX, fieldCenter.x - fieldWidth/2.0f, fieldCenter.x + fieldWidth/2.0f);
        targetZ = Mathf.Clamp(targetZ, fieldCenter.z - halfLength, fieldCenter.z + halfLength);

        // Debug.Log("clamp後 targetX: " + targetX + " targetZ: " + targetZ);
    ...
    public override void Cancel()
    {
        ...
        DebugLogger.Log($"[{_debugName}(RecievePassMyLeftSideActionRuntime)] パス受け移動キャンセル");
    }
} 