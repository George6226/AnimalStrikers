...
public class MoveToEnemyFieldActionRuntime : GoapActionRuntime
{
    ...
    public MoveToEnemyFieldActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        // ScriptableObjectを正しい型にキャスト
        var so = origin as MoveToEnemyFieldActionSO;
        if (so != null)
        {
            // 移動完了判定の有効半径を設定
    ...
    public override void Execute(PlayerBlackboard bb)
    {
        ...
        DebugLogger.Log($"[{_debugName}(MoveToEnemyFieldActionRuntime)] 敵陣地移動開始");
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (distance > _goodPositionRadius)
        {
            ...
            DebugLogger.Log($"[{_debugName}(MoveToEnemyFieldActionRuntime)] radian: {radian} direction: {direction} distance: {distance}");
        ...
        else
        {
            ...
            DebugLogger.Log($"[{_debugName}(MoveToEnemyFieldActionRuntime)] 敵陣地に到達: {_targetPosition}");
        }
    }
    ...
    private void UpdateTargetPosition()
    {
        ...
        // 自分の現在位置のX座標を保持し、Z座標のみ敵陣地側に設定
        float targetX = _playerBlackboard.PhysicalState.Position.x;   // 現在位置のX座標を保持
        float targetZ = fieldCenter.z +1.0f;  // 中央付近
    ...
    public override void Cancel()
    {
        ...
        DebugLogger.Log($"[{_debugName}(MoveToEnemyFieldActionRuntime)] 敵陣地移動キャンセル");
    }
} 