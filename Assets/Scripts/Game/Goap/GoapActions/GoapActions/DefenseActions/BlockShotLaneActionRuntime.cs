...
public class BlockShotLaneActionRuntime : GoapActionRuntime
{
    private const string DiagCategory = "BlockShotLane";
    private const string ActionName = "BlockShotLane";

    private bool _isExecuting;
    private float _startTime;
    private float _executionTime = 2.5f;
    private float _moveIntensity = 1f;
    private float _laneWidthRatio = 0.04f;

    private PlayerBlackboard _bb;
    private bool _motorResolved;
    private Vector3 _target;

    public BlockShotLaneActionRuntime(GoapActionSO origin, string debugName) : base(origin, debugName)
    {
        var so = origin as BlockShotLaneActionSO;
        if (so != null)
        {
            _executionTime = so.ExecutionTime;
            _moveIntensity = Mathf.Clamp(so.MoveSpeed / 4f, 0.5f, 1f);
            _laneWidthRatio = so.LaneWidthRatio;
        }
    }
    ...
    private Vector3 CalculateTarget(PlayerBlackboard bb)
    {
        ...
        float t = Mathf.Clamp01(Vector3.Dot(p - ownerPos, ao) / aoLenSq);
        Vector3 closest = ownerPos + ao * t;

        float laneWidth = teamBB.FieldInfo.FieldLength * _laneWidthRatio;
        Vector3 normal = Vector3.Cross(ao.normalized, Vector3.up);
        float side = Mathf.Sign(Vector3.Dot(p - closest, normal));
        Vector3 target = closest + normal * side * laneWidth * 0.5f;
