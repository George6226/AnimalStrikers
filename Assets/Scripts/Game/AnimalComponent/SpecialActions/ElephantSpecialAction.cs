...
// ゾウのスペシャルアクション
public class ElephantSpecialAction : AnimalSpecialActionBase
{
    [SerializeField] private AnimalFacade _myFacade;
    private string _barrierTeamTag = ConstData.ENEMY_TAG;
    ...
    public override void SetEffectCallback(GameObject effect)
    {
        ...
        var barrier = effect.GetComponent<SpecialBarrierEffect>();
        if (barrier == null){
            return;
        }
        ...
        {
            _barrierTeamTag = ConstData.PLAYER_TAG;
        }
        else
        {
            _barrierTeamTag = ConstData.ENEMY_TAG;
        }

        TeamFacade.Instance.TeamState.SetBarrierByTag(_barrierTeamTag, true);
        barrier.SetCallbackTarget(this);
    }
    public override void callBackEffect()
    {
        TeamFacade.Instance.TeamState.SetBarrierByTag(_barrierTeamTag, false);
    }
}
