...
echo ""
echo "  【手動操作について】"
echo "    GOAP 同士の試合のため M1/M2 / ルーズボール奪取中は手動入力オフ。"
echo "    キックオフ前（READY→GAME 前）は GAME 状態でないため操作不可。"
echo "    ルーズボールは最寄りの味方フィールド（Lion 含む）が MoveToFreeBall で奪取。"
...
echo "  見つからないときの切り分け:"
echo "    | ログの様子 | 意味 |"
echo "    | NoGoal のみ（キックオフ直後） | 最寄りでない → サポート/待機（正常） |"
echo "    | NoGoal のみ（試合中） | 味方/敵が保持中で FREE になっていない |"
