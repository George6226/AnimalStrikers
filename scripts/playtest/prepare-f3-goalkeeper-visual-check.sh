...
if [[ "${RESET_LOGS}" == "1" ]]; then
  ...
  echo "✅ GOAP / GK ログをリセット（GkDiag_latest.txt 含む）"
else
...
echo "  ※ GK 当たり判定ログ: GOAP_LOG_LEVEL>=1 で Assets/DebugLog/GkDiag_latest.txt"
echo ""
echo "--- Unity 手順 ---"
...
echo "  ログ直読み:"
echo "    Assets/DebugLog/GkDiag_latest.txt"
echo "  見るタグ: [GK_TRIGGER] [GK_PROBE] [GK_SAVE] [GK_CATCH] [GK_SKIP] [GK_COLLIDER]"
echo ""
echo "--- 確認後（任意） ---"
echo "  GOAP 回帰ログ:"
