...
GOAP_EDITMODE_EXPECTED_TESTS="${GOAP_EDITMODE_EXPECTED_TESTS:-98}"
...
GOAP_CI_MODES=(all editmode batch batch-combined batch-wing batch-cf-drive batch-defense batch-defense-tactical batch-defense-drive)
...
goap_ci_print_usage() {
  ...
  echo "  batch           上記6バッチ連続" >&2
  echo "  all             EditMode + 6バッチ（CI 相当・約12-16分）" >&2
}
...
goap_ci_mode_runs_batch() {
  case "${1}" in
    batch|all|batch-combined|batch-wing|batch-cf-drive|batch-defense|batch-defense-tactical|batch-defense-drive) return 0 ;;
    *) return 1 ;;
  esac
}
